...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void SaveActionBinding()
        {
            ...
            PlayerPrefs.SetString(m_Action.action.actionMap.name, currentBindings);
        }

        public void LoadActionBinding()
        {
            var saveBinding = PlayerPrefs.GetString(m_Action.action.actionMap.name);
            if (!string.IsNullOrEmpty(saveBinding))
            {
                actionReference.action.actionMap.LoadBindingOverridesFromJson(saveBinding);
            }
        }
        ...
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            foreach (var inputMap in action.actionMap.asset.actionMaps)
            {
                foreach (var existingAction in inputMap.actions)
                {
                    for (int i = 0; i < existingAction.bindings.Count; i++)
                    {
                        InputBinding existingBinding = existingAction.bindings[i];

                        if (existingAction == action && i == bindingIndex)
                        {
                            continue;
                        }

                        string existingBindingPath = existingBinding.effectivePath;

                        if (!string.IsNullOrEmpty(existingBindingPath) && existingBindingPath == newBindingPath)
                        {
                            Debug.Log($"Duplicado encontrado: '{newBindingPath}' ya está en uso por la acción '{existingAction.name}' (binding: '{existingBinding.path}')");
                            return true;
                        }
                    }
                }
